...
class AlpineDisplay:
    """Alpine Trading Bot Display System"""
    ...
    def update_display(self):
        ...
        try:
            ...
            status_text = Text("🏔️ Alpine Trading Bot", style="bold cyan")
            status_text.append(f" | Status: {'RUNNING' if self.running else 'STOPPED'}")
            status_text.append(f" | Time: {datetime.now().strftime('%H:%M:%S')}")
            
            panel = Panel(
                status_text,
                title="Alpine Trading Bot",
                border_style="cyan",
                box=box.ROUNDED
            )
