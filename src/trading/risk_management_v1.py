...
class RiskManager:
    ...
    def __init__(self, bitget_client=None):
        ...
        # Initialize BitgetClient if not provided
    ...
    def calculate_risk_metrics(self) -> RiskMetrics:
        ...
        try:
            # Get account info
            account_info = self.bitget_client.get_balance()
            if not account_info:
    ...
    def monitor_positions(self):
        ...
        try:
            ...
            for pos_data in positions:
                ...
                # Get position open time from trading engine or use current time as fallback
                position_open_time = self.get_position_open_time(symbol)
    ...
    def get_position_open_time(self, symbol: str) -> datetime:
        """Get position open time from trading engine or estimate"""
        try:
            # This would need to be connected to the trading engine
            # For now, we'll use a conservative estimate
            return datetime.now() - timedelta(hours=1)  # Assume 1 hour old if unknown
        except Exception as e:
            logger.error(f"Error getting position open time: {e}")
            return datetime.now()
    
    def should_close_position(self, position: Position) -> Tuple[bool, str]:
        """Check if a position should be closed based on time limits and other criteria"""
        try:
            ...
            if position_age > timedelta(hours=max_hold_hours):
                return True, f"Position held for {position_age.total_seconds()/3600:.1f} hours (max: {max_hold_hours}h)"
            
            # Check PnL limits
            if position.pnl_percentage < -5.0:  # 5% loss limit
                return True, f"Position loss exceeds 5%: {position.pnl_percentage:.2f}%"
            
            # Check if position is at +3% profit (take profit target)
            if position.pnl_percentage > 3.0:
                return True, f"Take profit target reached: {position.pnl_percentage:.2f}%"
            
            return False, "Position within acceptable limits"
            
        except Exception as e:
            logger.error(f"Error checking position closure criteria: {e}")
            return False, f"Error checking position: {e}"

# Global risk manager instance - deprecated, use RiskManager(bitget_client) instead
