...
from .risk_management_v1 import RiskManager, RiskLevel
...
class TradingEngine:
    ...
    def __init__(self):
        ...
        self.position_timestamps: Dict[str, datetime] = {}  # Track position open times
        ...
        # Initialize BitgetClient and RiskManager
        self.bitget_client = BitgetClient()
        self.risk_manager = RiskManager(self.bitget_client)
    ...
    def place_market_order(self, symbol: str, side: str, size: float, 
        ...
        ticker = self.bitget_client.get_ticker(symbol)
        if not ticker:
        ...
        can_open, reason = self.risk_manager.can_open_position(symbol, side, size, current_price)
        if not can_open:
    ...
    def place_limit_order(self, symbol: str, side: str, size: float, price: float,
        ...
        # Check risk management
        can_open, reason = self.risk_manager.can_open_position(symbol, side, size, price)
        if not can_open:
            logger.warning(f"Order rejected by risk manager: {reason}")
            return None
    ...
    def place_stop_order(self, symbol: str, side: str, size: float, stop_price: float,
        ...
        # Submit stop order
        try:
            result = self.bitget_client.place_stop_order(
                symbol=symbol,
                side=side,
                size=str(size),
    ...
    def _submit_order(self, order: Order, reduce_only: bool = False) -> Optional[Order]:
        ...
        try:
            # Submit to exchange
            result = self.bitget_client.place_order(
                symbol=order.symbol,
                side=order.side,
            ...
            if result and result.get('orderId'):
                ...
                # Track position timestamp for new positions (not reduce_only)
                if not reduce_only:
                    self.position_timestamps[order.symbol] = datetime.now()
                    logger.info(f"Position timestamp recorded for {order.symbol}")
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            result = self.bitget_client.cancel_order(order.symbol, order_id)
    ...
    def cancel_all_orders(self, symbol: str = None) -> bool:
        ...
        try:
            if symbol:
                result = self.bitget_client.cancel_all_orders(symbol)
                # Update local orders
                ...
                for sym in symbols:
                    self.bitget_client.cancel_all_orders(sym)
    ...
    def close_position(self, symbol: str, size: float = None) -> bool:
        ...
        try:
            ...
            positions = self.bitget_client.get_positions(symbol)
            if not positions:
    ...
    def close_all_positions(self) -> bool:
        ...
        try:
            positions = self.bitget_client.get_positions()
    ...
    def get_position_summary(self) -> Dict[str, Any]:
        ...
        try:
            positions = self.bitget_client.get_positions()
            balance = self.bitget_client.get_balance()
    ...
    def _monitor_positions(self):
        ...
        while self.is_running and not self._stop_event.is_set():
            try:
                ...
                # Check for positions that need to be closed due to time limits
                ...
                if risk_summary.get('risk_level') == 'critical':
                    ...
                    if risk_summary.get('daily_pnl', 0) < -self.config.risk_management.max_daily_loss:
                        logger.critical("Daily loss limit exceeded - considering emergency stop")
                        # self.risk_manager.activate_emergency_stop("Daily loss limit exceeded")
    ...
    def _check_position_time_limits(self):
        """Check and close positions that exceed time limits"""
        try:
            max_hold_hours = getattr(self.config, 'max_hold_time_hours', 12)
            current_time = datetime.now()
            
            positions_to_close = []
            
            for symbol, open_time in self.position_timestamps.items():
                position_age = current_time - open_time
                
                if position_age > timedelta(hours=max_hold_hours):
                    positions_to_close.append(symbol)
                    logger.warning(f"Position {symbol} has exceeded {max_hold_hours}h time limit ({position_age.total_seconds()/3600:.1f}h)")
            
            # Close positions that exceeded time limits
            for symbol in positions_to_close:
                try:
                    ...
                    if symbol in self.position_timestamps:
                        ...
                        logger.info(f"Removed position timestamp for {symbol}")
                except Exception as e:
                    logger.error(f"Error closing position {symbol}: {e}")
                    
        except Exception as e:
            logger.error(f"Error checking position time limits: {e}")
    ...
    def _update_order_status(self, order: Order):
        ...
        try:
            # Get open orders
            open_orders = self.bitget_client.get_open_orders(order.symbol)
            ...
            if not order_found:
                ...
                fills = self.bitget_client.get_fills(order.symbol, limit=50)
